...
public class UserCustomValidator {
	...
	public static boolean validate(User user, UserValidError userValidError) {
		...
		if(user.getId() == null || user.getId().trim().equals("") ) {
			...
			userValidError.setId("아이디 입력하셔야합니다. msg");
			result = false;
		}
		...
		if(user.getPw().length() < 4 || user.getPw().length() > 12) {
			userValidError.setPw("비밀번호 비번 8 12 확인 msg");
			result = false;
		}
		
		if(user.getName() == null || user.getId().trim().equals("") ) {
			userValidError.setName("이름을 입력하셔아합니다 msg");
			result = false;
		}
